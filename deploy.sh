...
# Make sure docker username is set
if [ $DOCKER_USERNAME = "" ]; then
    echo "Please set environment variable DOCKER_USERNAME=<docker username>"
    echo "images will be stored as <docker username>/<image name>:<tag name>"
    exit 0
fi
...
if [ $CMD = "start" ]; then
    ...
    VALIDATOR_TX="./${BINARY} tx staking create-validator --amount \"1000000000000000000000${STAKING_TOKEN}\" --pubkey=\"\$(./${BINARY} tendermint show-validator --home /${2})\" --moniker=$MONIKER --commission-rate=0.1 --commission-max-rate=\"0.20\" --commission-max-change-rate=\"0.01\"  --min-self-delegation=\"1\" FROM "
    if [ ${2} = "gaia" ]; then
    ...
    do
        ...
        $SUDO sed -i "s/enable = false/enable = true/g" "${HOMEDIRS[((${counter}))]}/config/app.toml"
        ((counter++))
    done
    ...
    do  
        if [ "${!counter}" = "--build" ]; then
            # Build a new docker image
             $SUDO docker build -f DockerfileChain -t ${DOCKER_USERNAME}/baton:${CHAIN_NAME} .
    ...
    do  
        if [ ${2} = "gaia" ]; then
            docker container create --name "${VALIDATORS[((${counter}))]}" --volume "${HOMEDIRS[((${counter}))]}:/${CHAIN_NAME}" --network "baton-net" --ip "192.255.${CHAINID_NUM}.$(( $counter + 2 ))" ${DOCKER_USERNAME}/baton:${CHAIN_NAME} ./${BINARY} start --home /${CHAIN_NAME} --grpc.enable true
        elif [ ${2} = "evmos"]; then
            docker container create --name "${VALIDATORS[((${counter}))]}" --volume "${HOMEDIRS[((${counter}))]}:/${CHAIN_NAME}" --network "baton-net" --ip "192.255.${CHAINID_NUM}.$(( $counter + 2 ))" ${DOCKER_USERNAME}/baton:${CHAIN_NAME} ./${BINARY} start --home /${CHAIN_NAME} --grpc.enable true --json-rpc.enable true
        fi
...
do
    ...
    # Usage
    USAGE="Usage: ./deploy.sh relayer <chain type> <chain-1-id> <chain-2-id> <node-1-id> <node-2-id> [OPTIONS]\n
    \t\"chain type\" can be "evmos" or "gaia"\n
    \tOptions:\n
...
port = 3001\n"
    ...
    do  
        if [ "${!counter}" = "--build" ]; then
            ...
            $SUDO docker build -f DockerfileRelayer -t ${DOCKER_USERNAME}/baton-relayer:latest .
    ...
    docker container create --name "${RELAYER_NAME}" --volume "$(pwd)/build/hermes:/hermes/:Z" --network "baton-net" --ip "192.255.255.$(ls -a | egrep '\.stop-' | wc -l)" ${DOCKER_USERNAME}/baton-relayer:latest
    ...
    if [ ${2} = "evmos" ]; then
        docker exec "${RELAYER_NAME}" /bin/bash -c "hermes --config config.toml keys add --hd-path \"${DERIVATION_PATH}\" --chain ${CHAIN_ID_PREFIX}-${CA} --key-file \"hermes/hermes-${CA}/rkey${NA}.json\""
        docker exec "${RELAYER_NAME}" /bin/bash -c "hermes --config config.toml keys add --hd-path \"${DERIVATION_PATH}\" --chain ${CHAIN_ID_PREFIX}-${CB} --key-file \"hermes/hermes-${CB}/rkey${NB}.json\""
    elif [ ${2} = "gaia" ]; then
        docker exec "${RELAYER_NAME}" /bin/bash -c "hermes --config config.toml keys add --chain ${CHAIN_ID_PREFIX}-${CA} --key-file \"hermes/hermes-${CA}/rkey${NA}.json\""
        docker exec "${RELAYER_NAME}" /bin/bash -c "hermes --config config.toml keys add --chain ${CHAIN_ID_PREFIX}-${CB} --key-file \"hermes/hermes-${CB}/rkey${NB}.json\""
    fi
...
# Setup connection for token transfers
# Ex. hermes --config config.toml create channel --a-chain evmos_9000-5 --b-chain evmos_9000-6 --a-port transfer --b-port transfer --new-client-connection
